#!/usr/bin/env python3
"""Bounded v9 qualification and explicitly selected development transfer runs.
...
from witness_cl.sql_env_v9 import make_stream, open_episode, evaluator_metadata
...
SOURCE_FILES = (
    ...
    'src/witness_cl/abstraction_v8.py', 'src/witness_cl/memory_v9.py',
    'src/witness_cl/model_v9.py', 'src/witness_cl/actions_v9.py',
    'experiments/competence_interactive_v9.py',
    'experiments/sql_abstractions_v9.py',
)
_COUNTERS = ('total_tokens', 'prompt_tokens', 'completion_tokens', 'calls',
...
class ResourceLimitsV9:
    ...
    def __post_init__(self):
        ...
        for name in ('ordinary_tokens', 'panel_tokens', 'ordinary_calls', 'panel_calls',
...
class _CombinedBudget:
    ...
    def __getattr__(self, name):
        ...
        if name == 'max_total_tokens':
            ...
            return min(self._local.max_total_tokens,
...
def execute_episode_v9(spec, memory, client, budget, *, phase, learn,
    ...
    if not isinstance(memory, ExperienceMemoryV9):
        raise ValueError('v9 memory with a shared solver prompt is required')
    ...
    trace = {'phase': phase, 'question': None, 'schema': None,
             ...
             'system_prompt_sha256': _text_hash(memory.system_prompt), 'learn': learn}
    conversation = []
        ...
        try:
            ...
            for _ in range(MAX_ACTIONS):
                content = client.complete(prefix + conversation, budget, phase=phase + ':solve',
                                          records=trace['model_calls'], output_tokens=solve_output_tokens, response_schema=ACTION_SCHEMA)
            ...
            trace['feedback'] = {'correct': trace['reward'] == 1.,
                                 'meaning': 'Correct on this episode only.' if trace['reward'] == 1.
            ...
            if learn:
                ...
                reflection = (_proposal_messages_v9(trace) if synthesizing and trace['reward'] == 1.
                ...
                if reflection is not None:
                    try:
                        reflection_text = client.complete(reflection, budget,
                                                          ...
                                                          response_schema=PROPOSAL_SCHEMA if synthesizing else reflection_schema_v9(memory.arm))
...
def _warm_result(traces, minimum):
    ...
    complete = (len(warm) == 8 and sorted(row['episode_index'] for row in warm) == list(range(8))
                and all(row['status'] in ('completed', 'no_valid_answer')
                        and not row.get('reflection_stop') for row in warm))
    correct = sum(row['reward'] == 1. for row in warm)
    return {'complete': complete, 'correct': correct, 'n': len(warm),
            'minimum_correct': minimum, 'meets_warm_accuracy_gate': complete and correct >= minimum}
...
def run_study(out: Path, client, *, stage='qualification', system_prompt=V9_SYSTEM,
              seeds=(92000,), conditions=('reuse',),
              arms=('full_history', 'insights', 'fragments'), limits=DEFAULT_LIMITS,
              minimum_warm_correct=7, gate_after_warm=True, stop_after=None):
    ...
    if (type(minimum_warm_correct) is not int or not 1 <= minimum_warm_correct <= 8
    ...
    if (not seeds or len(set(seeds)) != len(seeds)
            or any(type(seed) is not int or not 92000 <= seed <= 92003 for seed in seeds)
        ...
        raise ValueError('at most four unique development streams using seeds 92000..92003 are allowed')
    ...
    ExperienceMemoryV9(arms[0], system_prompt)
    ...
    client_config = {'model': client.model, 'context_tokens': client.context_tokens,
                     'max_output': client.max_output, 'response_mode': client.response_mode,
                     'endpoint': client.endpoint, 'timeout': client.timeout,
                     'decoding': deepcopy(client.decoding.to_dict())}
    expected = {'ordinary': 8 if stage == 'qualification' else 24,
                'old_before': 0 if stage == 'qualification' else 8,
                'old_after': 0 if stage == 'qualification' else 8,
                'final': 0 if stage == 'qualification' else 8}
    manifest = {
        'version': 9, 'kind': 'development_memory_qualification_and_transfer',
        'status': 'running', 'stage': stage, 'seeds': list(seeds),
        'conditions': list(conditions), 'arms': list(arms), 'limits': asdict(limits),
        'resource_scope': 'this invocation; prior diagnostics must be deducted by caller',
        'minimum_warm_correct': minimum_warm_correct, 'gate_after_warm': gate_after_warm,
        'system_prompt': system_prompt, 'system_prompt_sha256': _text_hash(system_prompt),
        'client_config': client_config, 'source_sha256': source_before,
        ...
        'started_unix': time.time(), 'stop_after_debug': stop_after,
        'native_benchmark': False, 'claim_confirmed': False,
    ...
    total = InferenceBudget(max_total_tokens=limits.total_tokens, max_calls=limits.total_calls,
                            deadline=deadline)
    ...
    try:
        for seed in seeds:
            for condition in conditions:
                ...
                memories = {arm: ExperienceMemoryV9(arm, system_prompt) for arm in arms}
                ordinary = {arm: InferenceBudget(max_total_tokens=limits.ordinary_tokens,
                                                 max_calls=limits.ordinary_calls, deadline=deadline) for arm in arms}
                panels = {arm: InferenceBudget(max_total_tokens=limits.panel_tokens,
                                               max_calls=limits.panel_calls, deadline=deadline) for arm in arms}
                ...
                for step, (phase, index, episode) in enumerate(_schedule(stream, stage)):
                    if step == 8 and stage == 'full' and gate_after_warm:
                        if not all(_warm_result(rows[arm], minimum_warm_correct)['meets_warm_accuracy_gate'] for arm in arms):
                            raise _QualificationStop('one or more configured arms missed the warm competence gate')
                    ...
                    for arm in order:
                        ...
                        trace = execute_episode_v9(
                            episode, memories[arm], paired_client, budget, phase=phase,
                            learn=phase == 'ordinary', solve_output_tokens=limits.solve_output_tokens,
                        ...
                        with (out / f'{seed}-{condition}-{arm}.jsonl').open('a', encoding='utf-8') as handle:
                        ...
                        print(canonical({'episode': count, 'seed': seed, 'condition': condition,
                                         ...
                                         'status': trace['status'], 'elapsed_seconds': round(time.monotonic() - started, 2)}), flush=True)
                        ...
                        if trace['status'] == 'resource_stop' or trace.get('reflection_stop'):
                            raise BudgetStop(trace.get('stop_reason') or trace.get('reflection_stop') or 'episode_resource_stop')
                        if stop_after is not None and count >= stop_after:
                            raise BudgetStop('explicit_debug_stop')
        manifest['status'] = 'completed'
    except Exception as exc:
        status = ('competence_gate_failed' if isinstance(exc, _QualificationStop)
                  else 'stopped' if isinstance(exc, BudgetStop) else 'failed')
        manifest.update(status=status, failure={'type': type(exc).__name__, 'reason': str(exc)[:512]})
    finally:
        all_traces = []
        for run_seed, run_condition, memories, ordinary, panels, rows in active_runs:
            for arm in arms:
                ...
                grouped = {phase: [trace for trace in traces if trace['phase'] == phase]
                           for phase in ('ordinary', 'old_before', 'old_after', 'final')}
                aggregate.append({
                    'seed': run_seed, 'condition': run_condition, 'arm': arm,
                    'ordinary_budget': ordinary[arm].to_dict(), 'panel_budget': panels[arm].to_dict(),
                    'memory': memories[arm].snapshot(), 'warm': _warm_result(traces, minimum_warm_correct),
                    'phase_counts': {phase: len(items) for phase, items in grouped.items()},
                    'phase_reward': {phase: sum(trace['reward'] for trace in items) / len(items) if items else None
                                     for phase, items in grouped.items()},
                    'phase_selects': {phase: sum(trace['select_attempts'] for trace in items) for phase, items in grouped.items()},
                    'phase_failures': {phase: sum(trace['status'] != 'completed' for trace in items) for phase, items in grouped.items()},
                })
        ...
        known_usage = all(isinstance(call.get('usage'), dict) and all(
            type(call['usage'].get(key)) is int and call['usage'][key] >= 0
            for key in ('prompt_tokens', 'completion_tokens', 'total_tokens'))
            and call['usage']['prompt_tokens'] + call['usage']['completion_tokens'] == call['usage']['total_tokens']
            for call in generated)
        usage_verified = (known_usage and len(generated) == total.calls and all(
            sum(call['usage'][key] for call in generated) == getattr(total, key)
            for key in ('prompt_tokens', 'completion_tokens', 'total_tokens')))
        resources_ok = (manifest['status'] not in ('stopped', 'failed')
                        and total.unknown_usage_calls == 0 and usage_verified and not any(
            trace['status'] in ('resource_stop', 'backend_or_runtime_failure') or trace.get('reflection_stop')
            for trace in all_traces))
        complete = (len(aggregate) == len(seeds) * len(conditions) * len(arms)
                    and all(row['phase_counts'] == expected for row in aggregate) and resources_ok)
        after = source_hashes()
        client_after = {'model': client.model, 'context_tokens': client.context_tokens,
                        'max_output': client.max_output, 'response_mode': client.response_mode,
                     'endpoint': client.endpoint, 'timeout': client.timeout,
                        'decoding': client.decoding.to_dict()}
        manifest.update(completed_episode_records=count, source_sha256_after=after,
                        source_unchanged=source_before == after, client_config_unchanged=client_after == client_config,
                        total_budget=total.to_dict(), contains_test_double_calls=simulated,
                        usage_verified=usage_verified,
                        required_records_complete=complete, resources_complete=resources_ok,
        ...
        if not manifest['source_unchanged'] or not manifest['client_config_unchanged']:
            manifest.update(status='invalidated', invalidation='executed_source_or_client_configuration_changed')
        ...
        manifest['warm_qualified'] = (manifest['status'] == 'completed' and complete and not simulated
        ...
        if not manifest['wall_cap_satisfied']:
            manifest.update(status='stopped', resources_complete=False, required_records_complete=False,
                            warm_qualified=False, failure={'type': 'BudgetStop', 'reason': 'global_wall_ceiling_including_export'})
        write_json(out / 'manifest.json', manifest)
    return manifest


run_pilot = run_study
execute_episode = execute_episode_v9
...
def main():
    ...
    parser.add_argument('--seeds', nargs='+', type=int, default=[92000])
    parser.add_argument('--conditions', nargs='+', choices=('reuse', 'nonreuse', 'near_match'), default=['reuse'])
    parser.add_argument('--arms', nargs='+', choices=ARMS, default=['full_history', 'insights', 'fragments'])
    parser.add_argument('--minimum-warm-correct', type=int, default=7)
    parser.add_argument('--no-warm-gate', action='store_true')
    parser.add_argument('--stop-after', type=int)
    for field in ('ordinary_tokens', 'panel_tokens', 'ordinary_calls', 'panel_calls',
                  'total_tokens', 'total_calls', 'solve_output_tokens', 'reflection_output_tokens'):
        parser.add_argument('--' + field.replace('_', '-'), type=int, default=getattr(DEFAULT_LIMITS, field))
    ...
    limits = ResourceLimitsV9(**{field: getattr(args, field) for field in ResourceLimitsV9.__dataclass_fields__})
    decoding = DecodingV9(temperature=args.temperature, top_p=args.top_p, top_k=args.top_k,
                          min_p=args.min_p, presence_penalty=args.presence_penalty,
                          seed=args.decoding_seed, thinking=not args.no_thinking)
    client = LocalInferenceV9(endpoint=args.endpoint, model=args.model, key_file=args.key_file,
                              context_tokens=args.context_tokens, max_output=max(limits.solve_output_tokens, limits.reflection_output_tokens),
                              timeout=args.timeout, decoding=decoding, response_mode=args.response_mode)
    system = args.system_prompt_file.read_text(encoding='utf-8') if args.system_prompt_file else V9_SYSTEM
    result = run_study(args.out, client, stage=args.stage, system_prompt=system,
                       seeds=tuple(args.seeds), conditions=tuple(args.conditions), arms=tuple(args.arms),
                       limits=limits, minimum_warm_correct=args.minimum_warm_correct,
                       gate_after_warm=not args.no_warm_gate, stop_after=args.stop_after)
    print(json.dumps({key: result.get(key) for key in ('status', 'warm_qualified', 'failure', 'completed_episode_records', 'elapsed_seconds')}, indent=2))
