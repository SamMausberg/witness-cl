...
Return exactly one action using the supplied response schema. When the schema
includes ACE generator fields, place this action in final_answer and include the
required reasoning and bullet_ids fields. Otherwise return the action directly.
    ...
    with open_episode(spec, allow_learning_checks=learn) as session:
        ...
        try:
            for action_index in range(MAX_ACTIONS):
                ...
                try:
                    decoded = memory.decode_generator(content) if hasattr(memory, "decode_generator") else content
                    action = parse_action(canonical(decoded) if isinstance(decoded, dict) else decoded)
